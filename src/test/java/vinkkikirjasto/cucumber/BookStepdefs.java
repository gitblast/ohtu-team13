...
public class BookStepdefs {
    ...
    @When("Author {string}, title {string}, pages {int} and published {int} are entered")
    public void nonEmptyAuthorAndTitleAreEntered(String url, String header, int pages, int published) {
        lisattavaKirja = new Book(url, header, pages, published);
        vinkkiService.addBook(lisattavaKirja);
    }
    
    @When("empty author and empty title with {int} pages and published {int} are entered")
    ...
    public void databaseContainsEnteredBook() {
        ...
        assertEquals(book.getJulkaisuvuosi(), lisattavaKirja.getJulkaisuvuosi());
    }
